...
const productSlice=createSlice({
    ...
    reducers:{
        addToCart(state,action)
        {
            ...
            if(existingProduct){
                existingProduct.totalPrice+=parseFloat(action.payload.quantities)*parseFloat(action.payload.price);
                existingProduct.quantities=parseFloat(existingProduct.quantities)+parseFloat(action.payload.quantities);
                state.totalPrice+=parseFloat(action.payload.quantities)*parseFloat(action.payload.price);
                ...
                state.productList.unshift(
                    {
                     ...
                     totalPrice:action.payload.price*action.payload.quantities
                    }
                    );
                    state.totalQuantity++;
